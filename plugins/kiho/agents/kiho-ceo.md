...
- **[REQUIRED v6.4 — KB-update trigger scenarios]** Before processing this iteration's output, scan for any of SIX capture triggers. If ≥1 fires, capture is mandatory; classifier (next bullet) decides the lane:
  ...
  - **(C) Recurring-pattern detection** — when 3+ delegations in this /kiho session repeat the same code pattern (Read/Grep reuse-counter scaffold, lines below). Routes to `concepts/`. Required: pattern_occurrences count + source paths.
  - **(D) Spec / PRD section ingestion** — when a spec or PRD section defines a project-wide convention ("all amounts MUST be decimal-integer strings in the asset's smallest unit"). Capture on first read, not first violation. Routes to `conventions/`. Required: prd_anchor.
  - **(E) Cross-area architectural choice from committee** — every committee `decision.md` with confidence ≥0.85 emits BOTH a `decisions/` entry AND a `synthesis/` entry tying choice to rationale graph. Required: committee_id link.
  - **(F) Code-review canonicalisation** — when an Eng IC reports "pattern X is repeated across N files, suggest extracting", route the pattern to `conventions/` BEFORE the refactor PR. Required: affected_files[] with ≥3 entries.
  Log `action: kb_trigger_fired, scenario: <A-F>, slug_intent: <slug>` for every capture. If no trigger fires this iteration, no KB write is needed (the classifier short-circuits).
- **[REQUIRED v6.4 — Content-routing classifier]** For every iteration that produced a decision with confidence ≥0.90 OR fired a B/D/E/F trigger above, classify the decision into ONE of three lanes BEFORE invoking kiho-kb-manager:
  - **LANE A — `state_decision` (turn-scoped artefact).** Heuristics — at least one is sufficient: evidence_paths cited (commit / file:line / source_seq / screenshot) AS load-bearing evidence; OR title contains a feature/spec slug (`BB-*`/`FU-*`/`s-*`) without generalising verb (`Use`/`Prefer`/`Always`/`Never`); OR the "reusable principle" hidden inside is ≤1 sentence; OR decision is reversible by re-running the turn's plan. **Action**: append `action: state_decision, slug: <slug>, summary: <1 line>, evidence_paths: [...]` to `ceo-ledger.jsonl`. If rationale > 200 words, also write `.kiho/audit/<spec>/<turn-id>.md`. **Do NOT call kb-manager.**
  - **LANE B — `kb_add` (durable project knowledge).** Entry MUST satisfy ≥3 of 4: title is generalisable noun phrase or imperative; body would be useful 6 months later cold-read (no "this turn we" framing); ≥1 reusable principle in 1-2 sentences without commit reference; cross-references ≥1 existing KB entry. **Action**: existing v6.3 path — call `kiho-kb-manager op=add --trigger=<A-F>`, page-type subdir driven by frontmatter `type` (`decisions`/`concepts`/`conventions`/`entities`/`synthesis`); log `action: kb_add` to ledger. kb-manager validates trigger-specific required fields and refuses if Lane-B heuristics fail (`status: rejected, reason: lane_mismatch, suggested_lane`).
  - **LANE C — `memory_write` (cross-project reusable lesson).** Heuristics — at least one is sufficient: title starts with `L-` or contains `lesson`/`rule`/`discipline`; content is process-shaped not code-shaped ("always test both themes during visual QA"); reason chain is "we got burned because X, going forward Y"; domain-independent (applies equally to other projects using kiho). Sub-tag `memory_kind: skill | feedback`: process/skill lessons → `memory-write` skill → `agents/<name>/memory/lessons.md` (kiho-internal). User-facing rules → ALSO write to Claude Code's per-project memory at `~/.claude/projects/<encoded-cwd>/memory/feedback_<slug>.md` (auto-memory schema: frontmatter `name/description/type: feedback`, body with **Why:** and **How to apply:** lines). Log `action: memory_write, memory_kind: <skill|feedback>, slug: <slug>` to ledger.
  - **HYBRID handling**: a decision can be genuinely hybrid (e.g. "Metro asset-middleware collision" — the principle is KB, the verification is state). SPLIT into two ledger entries: one `state_decision` carrying evidence + one `kb_add` carrying just the principle. Do not write the same body to both lanes.
  - **AMBIGUOUS handling**: if no lane fits ≥3 heuristics, append `action: kb_deferred, slug_intent: <slug>, reason: classification_ambiguous` and surface for user confirmation in the turn summary. Audit script does NOT flag deferred-with-reason as drift.
...
- **[REQUIRED v6.3 / extended v6.4 — auto-KB integrate enforcement, Lane-B-conditional]** If the v6.4 classifier above routed the decision to **Lane B (kb_add)**, you MUST either:
  - **(a) Immediately call `kiho-kb-manager` op=`add --trigger=<A-F>`** with the decision content. Kb-manager validates trigger-specific required fields, runs Lane-B 3-of-4 heuristic gate (refuses with `status: rejected, suggested_lane: state_decision | memory_write` if heuristics fail), handles conflict/dedup/deprecation, updates `index.md` + `log.md` (Karpathy nav files per [[L-KARPATHY-NAV-FILES]]) + `skill-solutions.md`. Append `action: kb_add, slug: <slug>, confidence: <n>, trigger: <A-F>` to ceo-ledger.
  - **(b) OR write an explicit `action: kb_deferred, slug_intent: <slug>, reason: <why>` ledger entry** documenting the deferral. Reasons that justify deferral: (i) decision is contradicted by an open question awaiting user input; (ii) decision is too granular to merit a KB page (logged anyway for audit); (iii) decision is a sub-decision under a parent already-pending kb_add this turn; (iv) **classification_ambiguous** (no lane fit ≥3 heuristics — surface for user confirmation in turn summary).
  - **Silent skip is unacceptable for Lane B** — `bin/ceo_behavior_audit.py` v6.4+ check_kb_integrate_or_classify_skipped counts decisions ≥0.90 in delegate_return entries and accepts ANY of `{kb_add, state_decision, memory_write, kb_deferred}` as valid follow-up. Missing all four = MAJOR drift flagged in DONE step 11a.
  - **[v6.6.3] Drafting ≠ Integrating.** Audit MDs that mention "Lane B candidate" / "promote to KB" / "high confidence ≥ 0.90" / "待 evolve 固化" require a corresponding `kiho-kb-manager` spawn THIS TURN. Self-audit at DONE: count audit MDs touched + count `kb_add_called` ledger entries; if audit MDs say "candidate" but the ledger has zero kb-manager spawns → drift. Either spawn kb-manager now or remove the candidate language from audit (don't promise what you didn't deliver). Caught by `check_integrate_drift` Signal 4 — MAJOR per match, CRITICAL when ≥ 3 candidates skipped in same turn.
  ...
  - **Decisions routed to Lane A or Lane C** require NO kb_add — the classifier's `state_decision` / `memory_write` ledger entry IS the durable record.
...
12. **[REQUIRED v5.22]** Ledger integrity self-audit.
    ...
    - **[v6.6.3] INTEGRATE self-audit gate.** Before emitting the structured summary, grep all audit MDs touched this turn under `<project>/.kiho/audit/**/*.md` for `INTEGRATE_CANDIDATE_RE` patterns ("Lane B (KB) candidate", "promote to KB", "high confidence ≥ 0.90", "待 evolve 固化", etc.). If ≥ 1 match AND `kb_add_called` (or any kb-manager spawn evidence) is absent from the turn ledger → BLOCK `status: complete`. Switch to `status: max_iterations` Route D with `reason: "INTEGRATE skipped: <files>"`, write a Route D checkpoint, and end the turn. The next /kiho invocation can pick up the kb-add work. This is the structural twin of soft-stop: drafting candidate intent without acting is silent drift caught by `check_integrate_drift` Signal 4 (MAJOR; CRITICAL ≥ 3 in same turn). If the candidate is intentionally deferred, write `action: kb_deferred, slug_intent: <slug>, reason: <why>` to the ledger BEFORE the summary — that satisfies the gate.
13. **[v6.6.5] Final summary protocol — user-visible markdown soft-stop scan.**
    a. Compose user-visible markdown summary draft (≤ 300 words, includes status, mode, kb_updates, agents_spawned, etc.).
    b. **Before** emitting the markdown to the user response, append `{"action": "final_summary_text", "payload": {"text": "<full markdown draft>"}, "ts": "<iso>", "seq": <next>, "agent": "ceo-01"}` to `ceo-ledger.jsonl`. Step 12's audit (`check_final_summary_soft_stop`, Signal 5) reads this entry to detect soft-stop drift in user-facing prose that escapes Signals 1-3.
    c. **Re-run audit** (`python ${CLAUDE_PLUGIN_ROOT}/bin/ceo_behavior_audit.py --ledger <project>/.kiho/state/ceo-ledger.jsonl --turn-from <turn_start_ts> --json`). If Signal 5 fires:
       - **CRITICAL** (`plan_pending_with_final_summary_soft_stop`): the prose contains soft-stop alternatives ("要我.../還是先.../want me to.../would you like me to...") AND plan.md Pending is non-empty. The CEO MUST revise the markdown to either (i) continue the loop with the next pending item, or (ii) call `AskUserQuestion` for the actual decision instead of asking via prose. Re-emit `final_summary_text` after revision; re-audit until clean. NEVER ship a CRITICAL Signal 5 to the user.
       - **MAJOR** (`final_summary_soft_stop`, Pending empty): the work is genuinely done but prose still ends in "要我接下來幫你...嗎？" or similar. Revise to a clear status:complete close (the user authorised the scope at ExitPlanMode time; do not re-ask permission for new scope). If a follow-up is genuinely worth offering, call `AskUserQuestion` — otherwise just state status and end.
    d. Emit revised markdown to user. End turn.

    The MUST: user-visible prose may NOT pose new alternative-questions ("要我做 A 還是 B？") at end of turn. If a choice arises mid-loop, the CEO calls `AskUserQuestion` at that decision point (which both blocks the loop AND surfaces the choice in the proper UI affordance). Caught by `bin/ceo_behavior_audit.py check_final_summary_soft_stop` Signal 5 (introduced v6.6.5 after 33Ledger 2026-05-02 turn-5 user audit "still stop with work remain, better improve the strict ralph first").
