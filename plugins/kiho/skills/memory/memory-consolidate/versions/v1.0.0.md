...
metadata:
  kiho:
    ...
    data_classes: ["observations", "reflections", "lessons"]
---
# memory-consolidate
...
4. If a behavioral pattern contradicts a soul trait by >2 points on the 1-10 scale, append a drift entry to the agent's `soul-overrides.md` file at `agents/<agent-id>/memory/soul-overrides.md`

### Drift entry format

```markdown
## [<iso-timestamp>] Trait drift detected
- Trait: <trait-name>
- Base score: <N>
- Observed behavior: <1-2 sentence description of the contradicting pattern>
- Suggested adjustment: <±N> (e.g., +2 or -3)
```
