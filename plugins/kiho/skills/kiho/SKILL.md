...
- **Do not draft Lane B candidates without spawning kb-manager.** Writing "high confidence ≥ 0.90" / "Lane B (KB) candidate" / "promote to KB" / "待 evolve 固化" inside a state/audit MD without spawning `kiho-kb-manager` in the same turn is the structural twin of soft-stop drift. v6.4 INTEGRATE rule says decisions ≥ 0.90 confidence MUST route through kb-manager during the loop, not be deferred to a future /kiho invocation. Caught by `bin/ceo_behavior_audit.py check_integrate_drift` Signal 4 as MAJOR per match (CRITICAL when ≥ 3 candidates skipped in the same turn).
